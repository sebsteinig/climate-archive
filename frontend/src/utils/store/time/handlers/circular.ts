...
async function nextCircularMean(time:Time,frame:TimeFrame,delta:number,active_variable:VariableName[]):Promise<TimeFrame> {
    const sync_frame = await sync(time,frame,active_variable)
    for( let [variable,data] of sync_frame.variables) {
            ...
            const new_idx = (time.exps.length + new_current.idx + didx) % time.exps.length
...
async function nextCircularTs(time:Time,frame:TimeFrame,delta:number,active_variable:VariableName[]):Promise<TimeFrame> {
    ...
    for( let [variable,data] of sync_frame.variables) {
        ...
        if (time.direction === TimeDirection.forward ) {
            [next_frame,next_chunk,next_idx,next_exp,next_info] = await peekNextTs(time.exps,variable,nb_c,fpc,new_current.frame,new_current.time_chunk,new_current.idx)
        }else {
            [next_frame,next_chunk,next_idx,next_exp,next_info] = await peekPreviousTs(time.exps,variable,nb_c,fpc,new_current.frame,new_current.time_chunk,new_current.idx)
        }

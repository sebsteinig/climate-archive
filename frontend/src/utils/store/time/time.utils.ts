...
import { TimeConf, TimeDirection, TimeFrame, TimeFrameState, TimeKind, WorldConf } from "./time.type";
...
export async function sync(frame:TimeFrame,variable:VariableName):Promise<TimeFrameState> {
...
export async function update(frame:TimeFrame,active_variables:VariableName[]) {
    const current_time = Math.floor(frame.weight)
    let next_time = current_time + 1
...
export function goto(frame:TimeFrame,to:number,duration?:number,onComplete?:()=>void) {
    ...
    return gsap.to(frame,{ease: "power3.out",
        duration:duration ?? 1,
        weight:to,
...
export function pin(frame:TimeFrame,onComplete?:()=>void){
    ...
    return gsap.to(frame,{weight:to,onCompleteParams:[frame],
        onComplete:(frame)=>{
            frame.swap_flag = true
            if(onComplete){
                onComplete()
...
export function circular(frame:TimeFrame,tween_ref:MutableRefObject<gsap.core.Tween|undefined>) {
    ...
    return gsap.to(frame,{duration:to,ease:"none",
        weight:to - 1,
        onCompleteParams:[frame,tween_ref],
        onComplete:(frame:TimeFrame,tween_ref:MutableRefObject<gsap.core.Tween|undefined>)=>{
            frame.weight = 0
            frame.swap_flag = true
...
export function once(frame:TimeFrame,tween_ref:MutableRefObject<gsap.core.Tween|undefined>,onComplete:()=>void) {
    const first = frame.variables.values().next().value as TimeFrameState | undefined
    if(!first) return undefined
    let state = {
        previous_idx : Math.floor(frame.weight)
...
export function walk(frame:TimeFrame,tween_ref:MutableRefObject<gsap.core.Tween|undefined>) {
    const first = frame.variables.values().next().value as TimeFrameState | undefined
    if(!first) return undefined
    const to = (frame.weight === first.info.timesteps - 1) ? 0 : first.info.timesteps - 1
    let state = {
        previous_idx : Math.floor(frame.weight)
    }
    return gsap.to(frame,{duration:first.info.timesteps,ease:"none",
        weight:to,
        onCompleteParams:[frame,tween_ref],
        onComplete:(frame:TimeFrame,tween_ref:MutableRefObject<gsap.core.Tween|undefined>)=>{
            frame.swap_flag = true

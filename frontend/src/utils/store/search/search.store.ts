...
> = (set) => {
  return {
    search: {
      ...
      togglePeriod() {
        set((state) => {
          state.search.filter.publication.year_span.period =
            !state.search.filter.publication.year_span.period
        })
      },
      setAuthor(author) {
      ...
      pushVar(variable) {
        set((state) => {
          ...
          if (state.search.filter.experiment.variables.includes(variable))
            return
      ...
      removeVar(variable) {
        set((state) => {
          const idx =
            state.search.filter.experiment.variables?.indexOf(variable)
          if (idx) {
      ...
      toggleLossless() {
        set((state) => {
          state.search.filter.experiment.lossless =
            !state.search.filter.experiment.lossless
        })
      },

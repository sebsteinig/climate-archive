        ...
        return(
            <div className={`bg-gray-900 mt-3 rounded-md 
                ...
                { displayed_collections.size > 1 &&                     
                    <Image onClick={() => setDisplayAll((prev) => !prev)}
                        priority
                        src={Dots}
                ...
                <div className='max-w-full overflow-y-auto overflow-x-hidden max-h-[80vh]' >                    
                    ...
                    Array.from(displayed_collections).map((value,idx) => {return (<div key={idx}>
                            <OneCollection onClick={()=> setDisplayDetails((prev : boolean) => {
                                setCurrentVariableControls(undefined)
                                return !prev})}
                                current_details={{collection : collections.get(value[0]), idx : value[0]}}
                                display_details = {display_details}/>
                            {displayed_collections.size > 1 && <MdSeparator className='ml-3 block self-center'/> }
                    </div>)}) }
                </div>
            </div>
        )
...
function OneCollection({current_details, display_details, onClick} : OneCollectioneProps){
    ...
    if(current_details.collection){
        return(
            <div key={current_details.idx}
            onMouseOver={() => setHover(true)} 
            onMouseLeave={() => setHover(false)}
            className={`${display_details&&"px-2"}`} >                
                <CurrentTitle onClick={onClick}
                            display_details={display_details} current_details = {{collection : current_details.collection, idx : current_details.idx}} />
                ...
                <ChangeData display_details={display_details} hover={hover} 
                    current_details = {{collection : current_details.collection, idx : current_details.idx}}/>                    
            </div>
        )
    }
}
...
function CurrentTitle({current_details, display_details, onClick} : CurrentTitleProps){
    return(
        ...
        <div className=''> 
            {isPublication(current_details.collection) && <div>                
                <p className="py-2 font-semibold text-sky-200">{current_details.collection.title}</p>
...
function CurrentPublication({publication} : {publication : Publication}){
    ...
    return(
        <div className={`p-2`}>    
            <p className="italic py-2 text-slate-400 text-sm">{publication.journal}, {publication.year}</p>
            <p className="font-medium tracking-wide">{publication.authors_full}</p>
            <div className="pt-2 pr-4" >
                <p className="pb-1 font-semibold">Abstract : </p>
                <p>{display_abstract? publication.abstract : publication.abstract.slice(0,90) + ' ...'}</p>

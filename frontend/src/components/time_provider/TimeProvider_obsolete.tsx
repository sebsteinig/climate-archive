...
import { TimeFirstResult, TimeMode, TimeFrame, TimeState } from "@/utils/store/time/time.type"
import { texture_provider } from "@/utils/texture_provider/TextureProvider"
import { findInTree } from "@/utils/store/texture_tree.store"
import { nextCircular } from "@/utils/store/time/time.utils";
...
export function TimeProvider(props:Props) {
    ...
    const setInitTimeFrame = useClusterStore((state) => state.time.set)
    ...
    let [time_result,setTimeFrame] = useState<TimeFrame | TimeFirstResult>(time_store.frames[0]?.current_result ?? {} as TimeFirstResult)
    useEffect(
        () => {
                                ...
                                if (time_store.frames[0] && Array.isArray(active_variable) && active_variable.length) {
                                    if (time_store.frames[0].state === TimeState.playing){
                                        ...
                                        if ( time_result ) { 
                                            ...
                                            time_result = await nextCircular(time_store.frames[0],time_result,delta,active_variable)
            ...
            <div className="absolute bottom-0 left-1/2 lg:-translate-x-1/2 w-1/2">
                ...
                <TimeController          
                    ...
                    pause = {() => {
                        ...
                        time_store.pause(0)
                        //setT(Math.floor(t))
                        setTimeFrame(time_result)
                        setInitTimeFrame(0,time_result)
                        return true

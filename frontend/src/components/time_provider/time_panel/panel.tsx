...
import { MutableRefObject, RefObject, forwardRef, useImperativeHandle, useRef } from "react"
...
export const Panel = forwardRef<Refs,PanelProps>(({time,time_idx},refs) => {
    ...
    const first_collection = time.collections.entries().next().value as [number,number]|undefined
    const is_alone = time.collections.size === 1 
        && first_collection && first_collection[1] === 1;
                ...
                is_alone ?
                        <Container 
                            ref={(el: HTMLDivElement) => {
                                ...
                                container_refs.current.set(first_collection![0], [elementsRef.current[0]])
                                return el
                            }}
                            time_idx={time_idx}

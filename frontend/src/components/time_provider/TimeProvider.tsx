...
export function TimeProvider(props:Props) {
    ...
    const active_variable = useMemo(() => {
        return Object.values(variables).filter((v)=> v.active)
    },[variables])
    const tree = useClusterStore(state => state.texture_tree)
    const [time_ref,setTime] = useTimeSlider()
    let [time_result,setTimeResult] = useState<TimeResult>()
    useEffect(
        () => {
                        ...
                        active_variable.map(
                            async (variable) => {
                                return {
                                    ...
                                    info:await texture_provider.getInfo(exps.exps[0],variable.name)
                                }
                            }
                        )
                    ).then(
                                ...
                                if (time_store.frames[0] && Array.isArray(active_variable) && active_variable.length) {
                                    if (time_store.frames[0].state === TimeState.playing){
                                        ...
                                        if ( time_result ) { 
                                            ...
                                            time_result = await time_store.frames[0].next(time_store.frames[0],time_result,delta,active_variable.map(e=>e.name))
            
                                            if (div_ref.current) {
                                                div_ref.current.textContent = time_result.get(active_variable[0].name)?.current.exp ?? ""
                                            }

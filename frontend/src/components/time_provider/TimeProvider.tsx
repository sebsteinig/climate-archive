...
export function TimeProvider(props:Props) {
    ...
    return (
        ...
        <div ref={container_ref} className="relative w-full h-full grid grid-cols-2 grid-rows-1 gap-4">
            {panels}
        </div>
    ...
    for(let [time_idx,time] of time_slots) {
        ...
        for(let [collection_idx,_] of time.collections) {
            ...
            if(time.state === TimeState.playing || time.state === TimeState.paused) {
                frame = await sync(time,collection.exps,saved_frames.get(time_idx)!.get(collection_idx)!,active_variables)
            }else {

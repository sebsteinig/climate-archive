    ...
    return (
      <div className={`flex gap-5 m-5 ${className ?? ''}`}>
        <div className="flex-grow overflow-hidden">
          <p className="text-emerald-600 font-semibold small-caps tracking-[.5em]" ref={time_title_ref}></p>
          <p className="truncate italic text-sm text-slate-600" ref={exp_title_ref}></p>
        </div>
        <div className="flex-grow flex gap-5 justify-center">
          { is_playing ? (
            ...
            <Pause
              ...
              onClick={() => {pause(tween_ref,setPlaying)}}
            />
          ) : (
            // PLAY BUTTON
            <Play
              className={`w-8 h-8 inline-block text-slate-500`}
              onClick={() => {play(data,time_id,current_frame,tween_ref,setPlaying)}}
            />
          )}
          <Stop
...
function play(
  data:WorldData,
  time_id: TimeID,current_frame: TimeFrameRef,
  tween_ref:MutableRefObject<gsap.core.Tween | undefined>,
  setPlaying:Dispatch<SetStateAction<boolean>>) {

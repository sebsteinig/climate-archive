...
export function Period() {
  const year_span = useStore(state => state.search.filter.publication.year_span)
    ...
    <Row >
      <Label>Year</Label>
          <ButtonSecondary
            onClick={() => {
              toggle()
            }}>{year_span.period ? "Between" : "Exactly"}</ButtonSecondary>
          { year_span.period ? 
            <>
              <div className="w-1/5">
                <Select
                ...
                >
                  {LOWER_PERIOD.map((year, index) => {
                    let year_value = year === DEFAULT_LOWER ? DEFAULT_LOWER_BOUND : parseInt(year)
                    return (
          ...
          <Select
            ...
            >
              {UPPER_PERIOD.map((year, index) => {
                let year_value = year === DEFAULT_UPPER ? new Date().getFullYear() : parseInt(year)
                return (
                  <option key={index} value={year_value}>
                    {year}
                  </option>

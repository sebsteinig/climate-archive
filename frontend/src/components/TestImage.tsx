...
export default function TestImage({}: Props) {
  ...
  const [texture_tree, current] = useClusterStore((state) => [
    state.texture_tree,
    state.current,
  ])
  ...
  useEffect(() => {
    ...
    const active_variables = Object.entries(variables)
      .filter(([name, slice]) => {
        return slice.active
      })
    ...
    if (current) {
      ...
      if (exp) {
        const tmp = active_variables
          .map((name) => exp.get(name))
          .filter((e) => e)
          .flat() as TextureLeaf[]

        const requests = tmp.map(async (texture) => {
    ...
    <div>
      ...
      <div className="grid grid-rows-12 grid-flow-col">
        {images.map((image, idx) => (
          <img key={idx} src={`data:image/jpeg;base64,${image.base64}`} />
        ))}
      </div>
    </div>
  )

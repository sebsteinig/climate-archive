...
export function buildTree(tree_repr: GeoTreeRepr): GeoTree {
  ...
  for (let [name, extracted_data] of Object.entries(
    root_extracted_data.branches!,
  )) {
    const data: GeoData = {
...
function populate(
  ...
  for (let [sub_name, sub_extracted_data] of Object.entries(
    extracted_data.branches!,
  )) {
    const data: GeoData = {
...
export function push(tree: GeoTree, to: Name, data: GeoData) {
  ...
  if (leaf.path.length === 0) {
    ...
    tree.root.branches.set(
      id,
      newBranch(tree.root.id, id, left_id, undefined, data),
    )
    // complete root leaf
...
function lastOf<K, V>(
  map: Map<K, V>,
): { left_id: K | undefined; left_value: V | undefined } {
